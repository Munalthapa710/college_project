...
# --- Models ---
# ... (Your models: RideRequest, User, Driver - remain the same) ...
class RideRequest(db.Model): # ... (as before)
...
class Driver(db.Model): # ... (as before)
    email = db.Column(db.String(120), primary_key=True)
    name = db.Column(db.String(100))
    phone = db.Column(db.String(15))
    password = db.Column(db.String(100)) 
...
# --- Decorators ---
def login_required_user(f): # ... (same) ...
    @wraps(f)
...
def login_required_driver(f): # ... (same) ...
    @wraps(f)
    def decorated_function(*args, **kwargs):
...
# --- CACHE CONTROL - REMOVED GLOBAL @app.after_request FOR NOW ---
# We will set headers per route for authenticated HTML pages

# --- Routes ---
@app.route('/')
def home(): return redirect(url_for('login'))

@app.route('/register', methods=['GET', 'POST']) # Public route, no special cache headers needed
def register():
...
@app.route('/login', methods=['GET', 'POST']) # Public route, no special cache headers needed
def login():
...
# --- API-like routes that return JSON (no need for HTML cache headers) ---
@app.route('/set-user-current-location', methods=['POST'])
@login_required_user
def set_user_current_location(): # ... (same logic as before) ...
    data = request.get_json(); # ... (rest of the logic)
...
@login_required_user
def find_nearest_driver_dijkstra(): # ... (same logic as before) ...
    data = request.get_json(); # ... (rest of the logic)
    if not data: return jsonify({'error': 'No JSON', 'success': False}), 400
...
@login_required_user
def request_ride(): # ... (same logic as before) ...
    data=request.get_json(); # ... (rest of the logic)
...
def accept_request(): # ... (same logic as before) ...
    data=request.get_json(); req_id=data.get('id') if data else None # ... (rest of the logic)
    if not req_id: return jsonify({'error': 'ID missing', 'success': False}), 400
    req = RideRequest.query.get(req_id)
    if not req or req.driver_email != session['driver'] or req.status != 'Pending': return jsonify({'error': 'Invalid req', 'success': False}), 400
    if req.user_latitude_at_request is None or req.user_longitude_at_request is None: return jsonify({'error': 'User location missing for ride.', 'success': False}), 400
    try: req.status = 'accepted'; db.session.commit(); return jsonify({'success': True, 'message': 'Ride accepted'}), 200
    except: db.session.rollback(); return jsonify({'error': 'DB err', 'success': False}), 500

@app.route('/reject-request', methods=['POST'])
@login_required_driver
def reject_request(): # ... (same logic as before) ...
    data=request.get_json(); req_id=data.get('id') if data else None # ... (rest of the logic)
    if not req_id: return jsonify({'error': 'ID missing', 'success': False}), 400
    req = RideRequest.query.get(req_id)
    if not req or req.driver_email != session['driver'] or req.status != 'Pending': return jsonify({'error': 'Invalid req', 'success': False}), 400
...
@login_required_driver
def complete_ride(): # ... (same logic as before) ...
    data=request.get_json(); ride_id=data.get('ride_id') if data else None # ... (rest of the logic)
    if not ride_id: return jsonify({'error': 'ID missing', 'success': False}), 400
    ride = RideRequest.query.get(ride_id)
    if not ride or ride.driver_email != session['driver']: return jsonify({'error': 'Invalid ride', 'success': False}), 400
...
def dashboard():
    ...
    if not driver or not driver.node or driver.node not in coordinates: 
        session.pop('driver',None); return "Driver profile error", 400
    # ... (logic to get pending_serialized and active_ride_serialized as before) ...
...
def user_dashboard():
    user = User.query.get(session['user'])
    # ... (logic to get driver_list_for_js and current_ride_status_info as before) ...
    ...
    status_info = None # Populate this correctly based on accepted, pending, rejected rides
...
def logout(): 
    ...
    # Explicitly set no-cache for the redirect TO the login page as well
    response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate, public, max-age=0"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "0"
    return response
